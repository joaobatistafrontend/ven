...
class AdicionarProdutosView(View):
    ...
    def post(self, request, venda_id):
        ...
        quantidade = dec(request.POST.get('quantidade'))

        produto = get_object_or_404(Produto, id=produto_id)
        
        # Confirma se produto é uma instância de Produto
        if not isinstance(produto, Produto):
            raise TypeError("O objeto produto não é uma instância do modelo Produto")

        # Verifica se produto.valor é um Decimal
        if not isinstance(produto.valor, Decimal):
